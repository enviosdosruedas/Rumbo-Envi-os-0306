...
export function OptimizadorRuta({ repartos }: OptimizadorRutaProps) {
  ...
  useEffect(() => {
    if (repartoSeleccionado) {
      ...
      if (reparto) {
        ...
        // Filtrar paradas que tienen envíos válidos y ordenar por orden actual
        const paradasValidas =
          reparto.paradas_reparto
            ?.filter((parada: any) => parada.envios && parada.envios.clientes)
            ?.sort((a: any, b: any) => a.orden - b.orden) || []

        setRutaOriginal(paradasValidas)
        setRutaOptimizada([])
  ...
  const calcularDistanciaTotal = (ruta: any[], puntoInicio?: { lat: number; lng: number }) => {
    if (ruta.length < 1) return 0

    let distanciaTotal = 0
    let puntoAnterior = puntoInicio || {
      lat: Number.parseFloat(ruta[0]?.envios?.clientes?.latitud || ruta[0]?.envios?.latitud_destino || "0"),
      lng: Number.parseFloat(ruta[0]?.envios?.clientes?.longitud || ruta[0]?.envios?.longitud_destino || "0"),
    }

    for (let i = 0; i < ruta.length; i++) {
      const puntoActual = {
        lat: Number.parseFloat(ruta[i]?.envios?.clientes?.latitud || ruta[i]?.envios?.latitud_destino || "0"),
        lng: Number.parseFloat(ruta[i]?.envios?.clientes?.longitud || ruta[i]?.envios?.longitud_destino || "0"),
      }
  ...
  const optimizarRuta = () => {
    if (!repartoActual || rutaOriginal.length < 2) {
      toast({
        title: "No se puede optimizar",
        description: "Se necesitan al menos 2 paradas para optimizar la ruta.",
        variant: "destructive",
      })
      return
    }
    ...
    setTimeout(() => {
      try {
        // Punto de inicio (configuración de empresa)
        let puntoInicio = null
        if (
          repartoActual.configuracion_empresa &&
          repartoActual.configuracion_empresa.latitud &&
          repartoActual.configuracion_empresa.longitud
        ) {
          puntoInicio = {
            lat: Number.parseFloat(repartoActual.configuracion_empresa.latitud),
            lng: Number.parseFloat(repartoActual.configuracion_empresa.longitud),
          }
        }
        ...
        let puntoActual = puntoInicio || {
          lat: Number.parseFloat(
            paradasDisponibles[0]?.envios?.clientes?.latitud || paradasDisponibles[0]?.envios?.latitud_destino || "0",
          ),
          lng: Number.parseFloat(
            paradasDisponibles[0]?.envios?.clientes?.longitud || paradasDisponibles[0]?.envios?.longitud_destino || "0",
          ),
        }
        ...
        if (!puntoInicio && paradasDisponibles.length > 0) {
          ...
          puntoActual = {
            lat: Number.parseFloat(
              rutaOptima[0]?.envios?.clientes?.latitud || rutaOptima[0]?.envios?.latitud_destino || "0",
            ),
            lng: Number.parseFloat(
              rutaOptima[0]?.envios?.clientes?.longitud || rutaOptima[0]?.envios?.longitud_destino || "0",
            ),
          }
        }
        ...
        while (paradasDisponibles.length > 0) {
          ...
          for (let i = 0; i < paradasDisponibles.length; i++) {
            ...
            const lat = Number.parseFloat(parada?.envios?.clientes?.latitud || parada?.envios?.latitud_destino || "0")
            const lng = Number.parseFloat(parada?.envios?.clientes?.longitud || parada?.envios?.longitud_destino || "0")

            const distancia = calcularDistancia(puntoActual.lat, puntoActual.lng, lat, lng)
          ...
          if (indiceMasCercano !== -1) {
            ...
            puntoActual = {
              lat: Number.parseFloat(
                paradaMasCercana?.envios?.clientes?.latitud || paradaMasCercana?.envios?.latitud_destino || "0",
              ),
              lng: Number.parseFloat(
                paradaMasCercana?.envios?.clientes?.longitud || paradaMasCercana?.envios?.longitud_destino || "0",
              ),
            }
          }
        }
            ...
            {repartoActual && (
              <div className="space-y-4">
                ...
                <MapaOptimizacion
                  ...
                  puntoInicio={
                    repartoActual.configuracion_empresa?.latitud && repartoActual.configuracion_empresa?.longitud
                      ? {
                          lat: Number.parseFloat(repartoActual.configuracion_empresa.latitud),
                          lng: Number.parseFloat(repartoActual.configuracion_empresa.longitud),
                          nombre: repartoActual.configuracion_empresa.nombre_empresa || "Empresa",
                        }
                      : undefined
                  }
                />
